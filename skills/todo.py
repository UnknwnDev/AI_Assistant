...
class Todo_skill():
	...
	def add_todo(self, olivia:AI)->bool:
		item = Item()
		try:
				item.title = olivia.listen()
	...
	def list_todos(self, olivia:AI):
		if len(self.todo) > 0:
				for item in self.todo:
						olivia.say(item.title)
		else:
				olivia.say("The to do list is empty!")

	def remove_todo(self, olivia:AI)->bool:
		try:
				item_title = olivia.listen()
